...
<head>
  ...
  <style>
    ...
    video { width: 60%; max-width: 600px; }
    button { margin-top: 10px; padding: 10px; cursor: pointer; }
    #predictionText { margin-top: 20px; font-size: 1.2rem; font-weight: bold; }
    #progressBarContainer { width: 60%; height: 20px; background-color: #eee; margin: 10px 0; position: relative; }
    #progressBar { width: 0%; height: 100%; background-color: #4CAF50; }
    #feedbackButtons { margin-top: 20px; }
    .feedbackButton { margin: 0 10px; padding: 10px 20px; cursor: pointer; }
    #feedbackMessage { margin-top: 10px; font-size: 1rem; font-weight: bold; }
  </style>
</head>
<body>
  <div class="container">
  ...
  <script>
    ...
    function sendFrames(frames) {
      fetch("http://localhost/upload_frames", {
        method: "POST",
    ...
    function sendFeedback(recordingId, feedback) {
      // Reset feedback message visibility
      ...
      fetch("http://localhost/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recording_id: recordingId, feedback: feedback })
      })
      .then(response => {
        if (response.ok) {
        ...
        resetUI();
        setTimeout(() => {
          feedbackMessage.textContent = ""; // Clear feedback text after 2 seconds
        }, 2000);
      })
      .catch(error => {
